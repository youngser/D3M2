...
yp.test2 <- function()
{
    ...
    n <- 10
    m <- 3
    g1 <- erdos.renyi.game(n, .4)
    V(g1)$id <- sample(100,n)
#    (randperm <- c(1:m, m+sample(n-m)))
    (randperm <- c(sample(n-m), (n-m+1):n))
#    (randperm <- 1:n)
# [1]  6  2  7  1  4  3  5  8  9 10
#    g2 <- sample_correlated_gnp(g1, corr=1, p=g1$p, perm=randperm)
    g2 <- g1 #permute.vertices(g1, randperm)
    V(g1)$id <- sample(n)
    V(g2)$id <- sample(n)
    (matching <- cbind(V(g1)$id, V(g2)$id))
    isomorphic(g1, g2)
...
#    pair <- matching[1:m,]
    ...
    out <- sgm(B, A, pair, hard=FALSE, start="rsp")
    str(out)
    cbind(1:n, out$corr)
    obj(B,A,out$P) # 5

    out2 <- sgm(B, A, pair, hard=TRUE, start="rsp")
    str(out2)
    cbind(1:n, c(1:m, out2$corr))
    obj(B,A,out2$P) # 5
